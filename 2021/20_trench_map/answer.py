...
def enhance_image(img: np.ndarray, img_enh_algorithm: ImgEnhAlgorithm) -> np.ndarray:
    img = np.pad(img, [(3, 3), (3, 3)], mode="constant", constant_values=False)
    img_enh = img.copy()
    ...
    for row in range(img_enh.shape[0]):
        if np.sum(img_enh[row, :]) > 0:
            if row > 0:
                img_enh = img_enh[row:, :]
            break
    for row in reversed(range(img_enh.shape[0])):
        if np.sum(img_enh[row, :]):
            if row < img_enh.shape[0] - 1:
                img_enh = img_enh[: row + 1, :]
            break
    for col in range(img_enh.shape[1]):
        if np.sum(img_enh[:, col]):
            if col > 0:
                img_enh = img_enh[:, col:]
            break
    for col in reversed(range(img_enh.shape[1])):
        if np.sum(img_enh[:, col]):
            if col < img_enh.shape[1] - 1:
